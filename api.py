...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("shilu.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated shilu.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update shilu.txt on VPS: {str(e)}")
