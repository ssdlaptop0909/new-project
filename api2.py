...
def update_soul_txt(public_url):
    with open("shilu2.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in shilu2.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("shilu2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated shilu2.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update shilu2.txt on VPS: {str(e)}")
